...
docker service create --replicas 1 --name cluster_service_1 -p 5011:5005 --mount type=bind,src=/opt/clusters/,dst=/clusters -e CLUSTER=1 65.108.82.162:8090/cluster
...
docker service create --replicas 1 --name cluster_service_3 -p 5013:5005 --mount type=bind,src=/opt/clusters/,dst=/clusters --constraint node.hostname==kcloud-production-user-107-vm-306 -e CLUSTER=3 65.108.82.162:8090/cluster